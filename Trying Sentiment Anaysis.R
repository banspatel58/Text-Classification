...
word_sentiment = remove_stopwords %>% inner_join(get_sentiments())
head(word_sentiment)

sentiment_df = word_sentiment %>% group_by(word) %>% 
  summarise(sentiment = sum(score), unique_words = n())
head(sentiment_df)

# Adding a column in r that computes the ratio of the number of word
sentiment_df = transform(sentiment_df, total = sentiment * unique_words)

data = Corpus(VectorSource(sentiment_df$word))
